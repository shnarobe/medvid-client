...
class RtspService extends EventEmitter {
    ...
    /**Read configuration from file */
        loadConfig() {
            ...
            try{
                const filePath = "../config.txt";//path.join(__dirname, "config.txt");
    ...
    async start() {
        try {
			...
			*/
            //monitor the config file for any possible changes path.join(__dirname,"config.txt")
			chokidar.watch("../config.txt").on("change",(event,path)=>{
	...
	try{
        return new Promise((resolve) => {
            ...
            ffmpeg_process.stdout.on("data",(data)=>{
				console.log("data received,standadr out");
				dataReceived=true;
